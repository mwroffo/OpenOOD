...
class GRAMPostprocessor(BasePostprocessor):
    ...
    def postprocess(self, net: nn.Module, data: Any):
        preds, deviations = self.get_deviations(net, data, self.feature_min,
    ...
    def tensor2list(self, x):
        return x.data.to(self.device).tolist()

    @torch.no_grad()
    def sample_estimator(self, model, train_loader, num_classes, powers):
        ...
        for batch in tqdm(train_loader, desc='Compute min/max'):
            ...
            for layer_idx in range(num_layer):
                ...
                for pole_idx, p in enumerate(num_poles_list):
                    ...
                    temp = temp**p
                    temp = temp.reshape(temp.shape[0], temp.shape[1], -1)  ## IndexError: tuple index out of range
    ...
    def get_deviations(self, model, data, mins, maxs, num_classes, powers):
        model.eval()
